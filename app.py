...
SECTORS = {
    "ERCOT BESS / Grid Storage": {
        ...
        "incidents": {"P1-BESS-01": {
            ...
            "audit_log": [{"ts": "2026-09-11 08:00:12 UTC", "hash": "8f3a9e...01c4", "event": "INCIDENT INITIALIZED: Elena Rostova withheld sign-off on Check #6."}, {"ts": "2026-09-11 09:15:40 UTC", "hash": "b2c174...aa91", "event": "SURVEILLANCE AGENT: Contractor standby carry activated at $1.01/sec."}]}}
    },
...
with st.sidebar:
    ...
    active_sector = st.selectbox("Operating Book (Top 12 Sectors):", list(st.session_state.app_state.keys()))
...
if selected_role.startswith("Director:"):
    ...
    inc_id = next((k for k, v in sector["incidents"].items() if v["director_seat"] == director_name), None)
    if not inc_id:
        st.title(f"Director Desk | {director_name}")
        st.success(f"Nominal: No active friction points under {d_meta['seat']}.")
    else:
        inc = sector["incidents"][inc_id]
    ...
    k3.metric("Critical Blockages", f"{len(active_incidents)} Active", "P1 Escalation", delta_color="inverse")
    primary_inc_id = list(sector["incidents"])[0]
    p_inc = sector["incidents"][primary_inc_id]
    if p_inc["status"] == "DEADLOCKED": st.error(f"""🚨 **MASTER ORCHESTRATOR INTERRUPT:** Critical standoff on **{primary_inc_id} ({p_inc['title']})**.
Contractor idle carry is bleeding **${p_inc['burn_rate_sec']:.2f}/second**. Holding costs will exceed total asset warranty value in **13.8 days**. Fiduciary review requires immediate arbitration.""")
    with st.container(border=True):
        st.markdown("### 🌐 Directorate Collaboration & Evidence Suite")
        g1, g2, g3, g4 = st.columns(4)
        links = [(g1, "https://meet.google.com/lookup/aat-" + primary_inc_id.lower() + "-arbitration", "📹 Instant Meet Bridge", "#1a73e8", f"Room: `aat-{primary_inc_id.lower()}`"), (g2, "https://notebooklm.google.com", "📓 NotebookLM Dossier", "#161b22", "ERCOT IA § 4.2 & Warranty Docs"), (g3, "https://drive.google.com", "📁 Telemetry Drive Vault", "#161b22", "Raw PSCAD & SCADA Evidence"), (g4, "https://docs.google.com", "📄 Board Resolution Doc", "#161b22", "Auto-Drafted Waiver Template")]
    ...
    with st.container(border=True):
        ...
        if q_text:
            ql = q_text.lower()
            if "bleed" in ql or "cost" in ql: st.info(f"**Master Agent ➔ Financial Surveillance:** Current bleed rate is **${p_inc['burn_rate_sec']:.2f}/sec** (${p_inc['burn_rate_sec']*604800:,.0f}/wk). Contractor carry has accrued **$184,320**. Inaction breaches the 13.8-day crossover threshold on Day 14, exceeding the entire $1.2M transformer value.")
    ...
    with t4_col1:
        ...
        with st.container(border=True):
            ...
            if "Option A" in choice:
                ...
                if p_inc["status"] == "DEADLOCKED" and st.button("Execute Directorate Carve-Out & Lock Ledger", use_container_width=True):
                    p_inc["status"], p_inc["burn_rate_sec"] = "RESOLVED", 0.0
                    for gm in p_inc["gms"].values():
                        for c in gm["checks"]: c["status"], c["evidence"] = "CLEARED", "Directorate Override"
                    record_ledger_entry(p_inc, "CHAIRMAN DIRECTIVE: Executed Option A (Indemnity Carve-Out). Elena Rostova indemnified. Check #6 cleared. Burn halted to $0/sec.")
                ...
                if p_inc["status"] == "DEADLOCKED" and st.button("Authorize Emergency Capital Draw", use_container_width=True):
                    record_ledger_entry(p_inc, "CHAIRMAN CAPITAL DRAW: $35,000 authorized for emergency IEEE 2800 on-site testing team."); st.success("Emergency Capital Released. Ledger Updated."); st.rerun()
            else:
